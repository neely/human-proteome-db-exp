...
- FASTA/database sequence-count summary
...
The FASTA sequence-count summary is provided in:

```text
derived/sequence_counts/database_sequence_counts.csv
```

This file was generated by counting FASTA header lines beginning with `>` using:

```text
scripts/utilities/seqCounts.py
```
...
derived/in_silico_tryptic_digest/unique_peptides/
```
...
The in silico tryptic digest can be regenerated from the OSF FASTA archive using:

```text
scripts/posthoc/tryptic_compare.py
```

The full peptide matrix generated by that script should remain outside GitHub and be archived on OSF.
...
- `results.json`
- `MSstats-Sage.csv`
- the full in silico tryptic `peptide_matrix.csv`
