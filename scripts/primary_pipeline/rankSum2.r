# Can run from command line with Rscript ranksum2.r (assume R/bin in PATH)
...
# Calculate the number of rows in "results.csv" (excluding headers)
num_rows <- nrow(results) - 1  # Subtract 1 for excluding headers

# Count how many values in the "corrected_p_values" column are less than 0.05
significant_p_values <- sum(results$corrected_p_values < 0.05)

# Write the information to an output text file
output_info <- sprintf("Number of rows in 'results.csv': %d\nNumber of significant p-values (< 0.05): %d", num_rows, significant_p_values)
writeLines(output_info, "output_info.txt")